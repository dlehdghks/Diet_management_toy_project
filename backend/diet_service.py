...
def calculate_calories(user_data):
    ...
    return {
        "bmr": round(bmr), "tdee": round(tdee), "target_calories": round(target), "calculation_reason": reason,
        "evidence": {"source": "Mifflin-St Jeor & Mayo Clinic", "description": "전 세계 영양학계가 사용하는 표준 공식입니다.", "formula": "10W+6.25H-5A+(5 or -161)"}
    }
...
def scale_food(food, factor):
    ...
    # 중량 표시가 없는 경우 (예: 2개, 1/2개 등) 뒤에 배수 표시
...
def find_best_combination(target_cal, base_count):
    ...
    for _ in range(100):
        ...
        # 칼로리 타겟에 따라 1.0~2.0배 사이에서 스케일 조절
        # 한 끼 목표가 700kcal 이상이면 적극적으로 증량
        factor = 1.0
